...
+ 调度运行时（libnvinfer_dispatch.so/.dll）是一个小型填充程序库，可以加载`精简运行时`，并将调用重定向到该库。调度运行时能够加载精简运行库的旧版本，并与构建器的适当配置一起，可用于提供新版本的TensorRT和旧计划文件之间的兼容性。使用 `调度运行时` 与手动加载 `精简运行时` 几乎相同，但它检查API是否由加载的`精简运行时`实现，并在可能的情况下执行一些参数映射以支持API更改。    
