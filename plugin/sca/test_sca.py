...
class SpatialCrossAttention():
    ...
    def forward(self, 
        ...
        logger.info(pt("value", value)) # [6, 1336, 1, 256]  --permute--> [1, 6, 1336, 256]  --reshape--> [6, 1336, 256]
        t = value.view(bs * self.num_cams, l, self.embed_dims)
        value = value.permute(2, 0, 1, 3).reshape(
            bs * self.num_cams, l, self.embed_dims)
        
        logger.info(pt("equal >> ", torch.equal(t, value))) #  [1, num_cams, max_len, embed_dims]
        ...
        logger.info(pt("count[..., None]", 1/count[..., None])) # [1, 40000, 1]
