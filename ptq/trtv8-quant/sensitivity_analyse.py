...
from tools.partial_quantization.utils import module_quant_enable, module_quant_disable, model_quant_disable, quant_sensitivity_save
...
def quant_sensitivity_analyse(model_ptq, evaler):
    ...
    for k, m in model_ptq.named_modules(): # {name, nn.Module}
        if isinstance(m, quant_nn.QuantConv2d) or           \
            ...
            # current module can not be quantized, skip and continue
            continue
        
        # exec eval ----------------------------------    
        eval_result = evaler.eval(model_ptq)
        print(eval_result)
        print("Quantize Layer {}, result mAP0.5 = {:0.4f}, mAP0.5:0.95 = {:0.4f}".format(k, eval_result[0], eval_result[1]))
        #                        {module_name, mAP, mAP}
...
if __name__ == '__main__':
    ...
    qfile = "{}_quant_sensitivity_{}_calib.txt".format(os.path.basename(args.weights).split('.')[0], args.batch_size * args.batch_number)
